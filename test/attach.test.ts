...
async function runAttach(id?: string) {
	await (attachCommand as { run: (ctx: { args: Record<string, unknown> }) => Promise<void> }).run({
		args: { id },
	})
}
	...
	describe('without id (interactive picker)', () => {
		...
		it('shows picker when multiple running containers', async () => {
			...
			expect(ui.prompts.select).toHaveBeenCalledWith({
				...
				options: [
					{ value: 'swift-falcon', label: 'swift-falcon', hint: '/home/user/project' },
					{ value: 'bold-otter', label: 'bold-otter', hint: '/home/user/project' },
				],
			})
